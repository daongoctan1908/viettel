...
class RobustBaseLLMController(ABC):
    ...
    def check_connectivity(self):
        ...
        try:
            response = self.get_completion("Reply with exactly one word: READY", temperature=0.0, max_tokens=8)
...
class RobustOpenAIController(RobustBaseLLMController):
    def __init__(
        ...
        self.default_max_tokens = int(os.getenv("OPENAI_DEFAULT_MAX_TOKENS") or 1000)
...
class RobustOllamaController(RobustBaseLLMController):
    ...
    def get_completion(self, prompt: str, temperature: float = 0.7, max_tokens: Optional[int] = None) -> str:
        ...
        response = chat(
            ...
            options={"temperature": temperature, "num_predict": max_tokens or 1000},
        )
...
class RobustLLMController:
    ...
    def __init__(self,
                 ...
                 candidate_extraction_max_tokens: Optional[int] = None,
                 update_decision_max_tokens: Optional[int] = None):
        if backend == "openai":
...
class RobustMemoryNote:
    ...
    def analyze_content(content: str, llm_controller: RobustLLMController) -> Dict:
        ...
        max_tokens = getattr(llm_controller, "candidate_extraction_max_tokens", None)
        try:
            response = llm_controller.llm.get_completion(prompt, max_tokens=max_tokens)
...
class RobustAgenticMemorySystem:
    ...
    def __init__(self,
        ...
        self.llm_controller.candidate_extraction_max_tokens = candidate_extraction_max_tokens
        self.llm_controller.update_decision_max_tokens = update_decision_max_tokens
        self.candidate_extraction_max_tokens = candidate_extraction_max_tokens
        self.update_decision_max_tokens = update_decision_max_tokens
        self.evo_cnt = 0
    ...
    def process_memory(self, note: RobustMemoryNote) -> tuple:
        ...
        try:
            ...
            decision_response = self.llm_controller.llm.get_completion(
                decision_prompt,
                max_tokens=self.update_decision_max_tokens,
            )
