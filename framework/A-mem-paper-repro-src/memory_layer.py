...
class OpenAIEmbeddingModel:
    """SentenceTransformer-compatible wrapper for OpenAI embedding models."""
    ...
    def __init__(self, model_name: str):
        ...
        max_retries = os.getenv("OPENAI_EMBEDDING_MAX_RETRIES") or os.getenv("OPENAI_MAX_RETRIES")
        timeout = os.getenv("OPENAI_EMBEDDING_TIMEOUT") or os.getenv("OPENAI_TIMEOUT")
        if max_retries:
...
def create_embedding_model(model_name: str):
    normalized_name = model_name.removeprefix("openai:")
    if normalized_name.startswith("text-embedding-"):
        return OpenAIEmbeddingModel(normalized_name)
    if SentenceTransformer is None:
        raise ImportError(
            "sentence-transformers with a working torch install is required for local embedding models. "
            "Use an OpenAI embedding model such as text-embedding-3-small to avoid this dependency."
        )
...
class HybridRetriever:
    ...
    def __init__(self, model_name: str = 'all-MiniLM-L6-v2', alpha: float = 0.5):
        ...
        """
        self.model = create_embedding_model(model_name)
        self.alpha = alpha
        self.bm25 = None
        self.corpus = []
